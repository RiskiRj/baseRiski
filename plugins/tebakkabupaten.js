...
`.trim()
    conn.tebakkabupaten[id] = [
        await conn.sendButtonImg(m.chat, await (await fetch(json.url)).buffer(), caption, '© 𝕭𝖔𝖞 𝕭𝖔𝖙', 'Bantuan', '.teka', m),
        json, poin,
        setTimeout(async () => {
            if (conn.tebakkabupaten[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.title}*`, '© 𝕭𝖔𝖞 𝕭𝖔𝖙', 'Tebak Kabupaten', '.tebakkabupaten', conn.tebakkabupaten[id][0])
